...
export default function ProfileCard({ user, API_BASE }) {
  ...
  const goToProfile = () => {
    if (user?.name) navigate(`/Profile/${encodeURIComponent(user.name)}`);
  };
  ...
  return (
    <div className="profilecard-container">
      ...
      {/* 🔹 Avatar section */}
      <div className="profilecard-avatar" onClick={goToProfile} role="button">
        <img src={avatarFull(user.avatar_url)} alt={user.name} />
      </div>

      {/* 🔹 Name + title */}
      <div className="profilecard-name" onClick={goToProfile} role="button">
        {user.name}
      </div>
      <div className="profilecard-title" onClick={goToProfile} role="button">
