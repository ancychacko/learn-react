...
//       FROM posts p
//       JOIN users u ON u.id = p.user_id
//       LEFT JOIN (SELECT post_id, COUNT(*)::int AS count FROM likes GROUP BY post_id) lc ON lc.post_id = p.id
//       LEFT JOIN (SELECT post_id, COUNT(*)::int AS count FROM comments GROUP BY post_id) cc ON cc.post_id = p.id
//       LEFT JOIN (SELECT post_id, COUNT(*)::int AS count FROM shares GROUP BY post_id) sc ON sc.post_id = p.id
//       WHERE p.id = $1
//       LIMIT 1;
//     `;
...
/**
 * Get posts list - respects visibility logic
 */
async function getPosts(req, res) {
  if (!req.session.userId)
    return res.status(401).json({ error: "Not authenticated" });
