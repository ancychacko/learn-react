...
/* ============================================================
   DELETE POST  — UPDATED WITH SHARE COUNT FIX
============================================================ */
async function deletePost(req, res) {
  if (!req.session.userId)
    return res.status(401).json({ error: "Not authenticated" });
  ...
  try {
    // Fetch post including is_share and original_post_id
    const r = await pool.query(
      "SELECT user_id, media_url, is_share, original_post_id FROM posts WHERE id=$1",
      [postId]
    );
    ...
    const isShare = post.is_share;
    const originalPostId = post.original_post_id;

    /* ------------------------------------------------------------
       1️⃣ IF THIS POST IS A SHARE → DECREMENT SHARE COUNT SAFELY
    ------------------------------------------------------------- */
    if (isShare && originalPostId) {
      await pool.query(
        "DELETE FROM shares WHERE post_id = $1 AND user_id = $2",
        [originalPostId, req.session.userId]
      );
    }

    /* ------------------------------------------------------------
       2️⃣ Remove references from other reposts
    ------------------------------------------------------------- */
    await pool.query(
      "UPDATE posts SET original_post_id=NULL WHERE original_post_id=$1",
      [postId]
